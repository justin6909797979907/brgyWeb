<div class="modal fade" id="createAnnouncement" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true" wire:ignore.self>
      ...
      <div class="modal-body">
        <form wire:submit='save'>
          ...
          <div class="mb-3">
            ...
            <input type="file" class="form-control @error('image') is-invalid   @else  @if(!empty($image)) is-valid  @endif @enderror"  accept="image/*" wire:model='image'>
            @error('image')
            <span class="text-danger">{{ $message }}</span>
           @enderror
            <div class="mt-2">
              @if ($image) 
              <img src="{{ $image->temporaryUrl() }}" class="img-fluid rounded" style="max-height: 208px;margin: 21px auto 0px auto;">
             @endif
            </div>
          </div>
          <div class="mb-3">
